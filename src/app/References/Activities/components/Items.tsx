...
import {IFormActivity} from '../Activities'
...
const Items: React.FC<FieldArrayRenderProps & Props> = props => {
    const items: IFormActivity[] = getIn(props.form.values, props.name)
    ...
    const saveItem = async (index: number) => {
        ...
        try {
            if (!item.id) {
                const response = await api.Activities.create({name: item.name})
                props.replace(index, response.activity)
                notificationsStore.addSuccess('Добавлена новая сфера работы')
            } else if (changedItems.includes(index)) {
                await api.Activities.update(item.id, {name: item.name})
    ...
    return <section className={cx('activities')}>
        <Button icon='plus' content='Добавить сферу работы' onClick={addItem} className={cx('btn-add')}/>

        {items.map((item, index) => {
            return <Grid templateColumns={'auto 40px'} key={index} className={cx('activity')}>
